...
export default function Home() {
  ...
  const handlePdfDrop = (e: React.DragEvent, index: number) => {
    e.preventDefault();
    e.stopPropagation();
    setIsDragOver(false);
                ...
                >
                  <div className="space-y-3">
                    ...
                    <p className="text-slate-500 text-sm">
                      Upload individual PDF files in separate spaces or use the bulk zone to upload all 5 at once.
                    </p>
                    ...
                    >
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <div>
                          <p className="text-slate-700 font-bold text-lg">Bulk PDF Upload Zone</p>
                          <p className="text-slate-500 text-sm">Drop up to 5 PDFs here to fill all slots alternatively</p>
                        </div>
                        <div className="bg-blue-600 text-white px-6 py-2 rounded-full text-sm font-semibold hover:bg-blue-700 transition-colors">
                      ...
                      {[1, 2, 3, 4, 5].map((num) => (
                        <div key={num} className="space-y-2">
                          ...
                          <div
                            ...
                            onDragOver={handleDragOver}
                            onDragLeave={handleDragLeave}
                            onDrop={(e) => handlePdfDrop(e, num - 1)}
                          >
                            {files[num - 1] ? (
...
function GeneratedIDCardPreview({ data, index, customFrontTemplate, customBackTemplate }: GeneratedIDCardPreviewProps) {
  ...
  const [saturation, setSaturation] = useState<number>(45);
